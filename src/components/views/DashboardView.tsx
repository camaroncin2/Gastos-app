...
export default function DashboardView() {
  const {
    ...
    getRemainingBalance: s.getRemainingBalance,
  ...
  const totalLoaned = useMemo(() => loans.reduce((s, l) => s + convertToCRC(l.amount, l.currency), 0), [loans, convertToCRC]);
  const totalLoanRecovered = useMemo(() => loans.reduce((s, l) => s + convertToCRC(l.payments.reduce((ps, p) => ps + p.amount, 0), l.currency), 0), [loans, convertToCRC]);
  const totalLoanPending = totalLoaned - totalLoanRecovered;
  const activeLoansCount = useMemo(() => loans.filter((l) => l.payments.reduce((s, p) => s + p.amount, 0) < l.amount).length, [loans]);
  ...
  ], [totalIncomes, totalExpenses, totalDebts, totalSavings]);
  ...
  }), [currentMonth, incomes, expenses, convertToCRC]);
